...
int main() {
	...
	for (int key_length : key_list) {
		...
		// init strings
		type_str = "type \t";
		bytes_per_sec = "aes-" + std::to_string(key_length * 8) + " cbc";
		...
		for (int byte_val : byte_list) {
			...
			CryptoPP::CBC_Mode<CryptoPP::Rijndael>::Encryption obj;
			...
			std::cout << "Doing aes-" << key_length * 8 << " cbc for 3s on " << byte_val << " size blocks: " << full_cycle << " aes-" << key_length * 8 << " cbc in " << std::format("{:.2f}", float(float(finish - start) / 1000)) << "'s" << std::endl;
