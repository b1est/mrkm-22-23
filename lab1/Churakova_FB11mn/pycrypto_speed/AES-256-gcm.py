...
for key in key_list:
    ...
    type_str = "type \t\t"
    bytes_per_sec = f"aes-{key*8} gcm\t"
    for byte in byte_list:
        msg = urandom(byte)
        start = time.time()
        full_cycle = 0
        obj = AES.new(secret_key, AES.MODE_GCM, iv)
        while time.time() - start < 3.0:
            encrypted_msg = obj.encrypt(msg)
            full_cycle += 1
        finish = time.time()
        print(f"Doing aes-{key*8} gcm for 3s on {byte} size blocks: {full_cycle} aes-{key*8} gcm's in {round(finish-start, 2)}'s")
